...
<?php // print("<pre>"); print_r($all_records); exit('view');?>
<!-- Content Wrapper. Contains page content -->
<div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
        <h1>{{ $gallery_name }}<small>Gallery</small></h1>
    </section>
    <!-- Main content -->
            ...
            <div class="col-xs-12">
                ...
                <div class="box box-primary">
                    ...
                    <div class="box-header with-border">
                        <div class="row">
                            ...
                            <div class="col-md-1 col-xs-4 text-right">
                                <a href="{!! url('admin/gallery') !!}" class="btn btn-info btn-flat pull-right">Back</a>
                            </div>
                        </div>
                    </div><!-- /.box-header -->
                    ...
                    <div class="box-body pad" style="min-height: 340px;">
                        ...
                        <div class="row">
                            <div class="col-xs-12">
                                <span class="help-block">(Dimension: Any & <?php echo $allowfiletypetext;?> : <?php echo $filetype;?>)</span>
                            </div>
                        </div>
                        <br>
                        <div class="row img-row" id="mediaCont">
                            @if(count($all_records) > 0)
                                @foreach($all_records as $row)
                                    <?php // echo '<pre>'; print_r($row); exit('row'); 
                                        $id = $row->id;
                                        $image = $row->image;
                                    ...
                                    <div class="col-md-2 img-col" id="img<?php echo $id; ?>">
                                        <div class="img-actions">
                                            <img src="{{ asset("gallery/images/thumbnail/$gallery_id/$image") }}" style="width:100%;">
                                            <a href="javascript:void(0)" class="view-img" data-toggle="tooltip" title="{{ $image }}"><i class="fa fa-search"></i></a>&nbsp;
                                            <?php
                                                // $userData = Session::get('user_data');
                                                // $roleId = (!empty($userData) ? $userData['gallery_id']:0);
                                                // if($roleId == 1){
                                            ?>
...
jQuery(document).ready(function(){
    var encGalleryId = '<?php echo base64_encode($galleryId); ?>';
    // var fileWidth = parseInt(1920);
    // var fileHeight = parseInt(1280);
    // jQuery
    ...
    uploader.on('sending', function(file, xhr, formData){
        formData.append('gallery_id', encGalleryId);
    });
        ...
        if(status=='1'){
            ...
            var htmlCont = '<div class="col-md-2 img-col" id="img'+data.id+'">'+
                                '<div class="img-container">'+
                                    '<img src="'+SITE_URL+'/gallery/image/'+data.image+'" class="img-responsive" title="" id="">'+
                                '</div>'+
                                '<div class="img-actions">'+
    ...
    //     var slider_name = '';
    //     var redirectUrl = '<a href="'+SITE_URL+'/admin/action-logs/history/slider-image-gallery/0/'+encRoundId+'" class="btn btn-xs btn-theme text-aqua" data-toggle="tooltip" title="View All Logs"><i class="fa fa-history"></i></a>';
    ...
    $("#mediaCont").on("click",".remove-img", function(){
        ...
        if(ele_id != ''){
            var reqUrl = SITE_URL+'/admin/gallery-image/delete';
