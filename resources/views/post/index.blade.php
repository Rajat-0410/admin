        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="box box-info">
                    ...
                    <div class="box-body">
                        {!! Form::open(['url' => 'admin/post', 'id' => 'addBookTestRide', 'method' => 'get', 'class' => '']) !!}
                            <div class="row">
                                <div class="col-md-4 col-xs-12">
                                    <div class="form-group has-feedback">
                                        {!! Form::label('category_id','Category') !!}
                                        {!! Form::select('category_id', $arrCategories, $category_id, ['class' => 'form-control', 'id' => 'parent_category']) !!}
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-12 col-xs-12">
                                    <p class="help-block"><strong>Search By:</strong> Title  OR Category OR Slug</p>
                                </div>
                            </div>
                            <div class="form-group">
                                {!! Form::submit('Search',['class' => 'btn btn-primary btn-med']) !!}&nbsp;&nbsp;
                                <a href="{!! url('admin/post') !!}" class="btn btn-default btn-med">Reset</a>
                            </div>
                        {!! Form::close() !!}
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-4 text-right">
                                <?php if(isset($add_action) && $add_action==true){ ?>
                                    <a href="{!! url('admin/post/add') !!}" class="btn btn-primary btn-flat pull-right">Add New Post</a>
                                <?php } ?>
                            </div>
                        </div>
                    </div><!-- /.box-header -->
                            ...
                            @if(count($all_records) > 0)
                                ...
                                @foreach($all_records as $row)
                                    <?php // print_r($row); exit; 
                                        ...
                                        $categoryName = (!empty($row['post_category']->name) ? $row['post_category']->name:'N/A');
                                        $status = $row->status;
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center" width="10%">
                                            <a href="javascript:void(0)" class="change-status" id="post-<?php echo base64_decode($id); ?>" data-id="<?php echo $id; ?>" data-status="<?php echo ($status==1) ? '0':'1'; ?>" data-model="Post"><span class="label <?php echo $statusClass; ?>"><?php echo $statusText; ?></span></a>
                                        </td>
                                        <td>{{ date('Y/m/d',strtotime($row->publish_date)) }}</td>
                                        <td>
                                            <?php if(isset($edit_action) && $edit_action==true){ ?>
                                            <a href="{!! url('admin/post/edit', ['id' => $id]) !!}" class="btn btn-xs btn-theme"><i class="fa fa-pencil"></i></a>
                                            <?php } ?>
        ...
        if(ele_id != ''){
            bootbox.confirm({
                ...
                callback: function (result) {
                    ...
                    if(result){
                        location.href = SITE_URL+'/admin/post/delete/'+ele_id;
                    }
                }
            });
        }
